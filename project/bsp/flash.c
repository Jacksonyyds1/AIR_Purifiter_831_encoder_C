...
{
    uint32_t dataTemp = 0;
    flash_read_word(read_addr, &dataTemp, 1);
    return dataTemp;
}
/**
...
{
    uint16_t dataTemp = 0;
    flash_read(read_addr, &dataTemp, 1);
    return dataTemp;
}
...
{
    uint8_t dataTemp = 0;
    flash_read_byte(read_addr, &dataTemp, 1);
    return dataTemp;
}
/**
...
error_status flash_write_nocheck_word(uint32_t write_addr, uint32_t *p_buffer, uint16_t num_write)
{
    uint16_t i;
    flash_status_type status = FLASH_OPERATE_DONE;
    for(i = 0; i < num_write; i++)
...
error_status flash_write_byte(uint32_t write_addr, uint8_t *p_buffer, uint16_t num_write)
{
    ...
    if(flash_write_nocheck_byte(write_addr, p_buffer, num_write) != SUCCESS)
    {
        return ERROR;
    }
...
error_status flash_write_halfword(uint32_t write_addr, uint16_t *p_buffer, uint16_t num_write)
{
    flash_status_type status = FLASH_OPERATE_DONE;
    ...
    if(flash_write_nocheck(write_addr, p_buffer, num_write) != SUCCESS)
    {
        return ERROR;
    }
...
error_status flash_write_word(uint32_t write_addr, uint32_t *p_buffer, uint16_t num_write)
{
    ...
    if(flash_write_nocheck_word(write_addr, p_buffer, num_write) != SUCCESS)
    {
        return ERROR;
    }
