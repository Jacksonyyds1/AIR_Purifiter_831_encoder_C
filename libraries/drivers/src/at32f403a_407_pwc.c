...
void pwc_deepsleep_mode_enter(void)
{
...
/* clear second flag */
  ...
  rtc_alarm_set(rtc_counter_get() + 1);
...
//    at32_led_off(LED2);
  ...
  delay_us(120);
	...
	if(core_get_tek_power()->deepsleepModeFlag && (!core_get_tek_power()->deepsleepModeFirstWakeupFlag))
	{
		core_get_tek_power()->deepsleepModeFirstWakeupFlag=0;
		core_get_tek_power()->deepsleepModeFlag = 0;
		pwc_deepsleep_mode_enter();
	}
